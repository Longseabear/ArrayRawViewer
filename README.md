...
Array 탭 복귀는 캐시된 이미지와 설정만 복원하며 디버거 메모리를 읽지 않습니다.
Frame map 캐시가 없으면 Capture로 데이터를 갱신할 때 다시 생성합니다.

같은 옵션 페이지의 **Search timeout (sec)**에서 검색 제한 시간을 설정하고
**Save**로 저장합니다. 기본 10초, 범위 1~120초이며 모든 솔루션에 적용됩니다.
돋보기 아이콘을 누르면 같은 버튼이 취소 아이콘으로 바뀌고 `검색 중…`을 표시합니다.
입력창과 Viewer 색상은 유지하며 별도 로딩 막대를 추가하지 않습니다.
취소는 디버거 호출 사이에서 처리됩니다. 검색 중 루트를 바꾸면 진행 중인 결과는 적용하지 않습니다.
시간 제한은 디버거 호출 사이에서 확인하므로 단일 COM 호출이 지연되면
실제 종료가 늦어질 수 있습니다.

Search objects는 매번 현재 솔루션의 저장된 옵션 정의를 다시 읽고 현재 객체를 검색합니다.
Remember for solution을 꺼도 구조체 정의는 읽습니다. 기존 검색 결과 캐시를 재사용하지
않으므로 스택 프레임을 바꾼 뒤에도 이전 결과가 그대로 반환되지 않습니다.
루트 평가 결과를 재사용하고 같은 타입의 매칭 결정을 검색 안에서 캐싱합니다.
UI 갱신은 항목마다 대기하지 않고 약 40ms 간격으로 실행 기회를 양보합니다.
주소를 명확히 확인할 수 있는 단일 포인터는 타입과 주소 기준으로 중복 확장을 막고,
주소를 해석할 수 없으면 기존 깊이·노드·시간 제한 아래에서 탐색을 유지합니다.

기반 클래스 항목은 검색하지 않습니다. 네이티브 DTE가 노출하는 타입명과 같은 이름의
비포인터 자식 항목(예: 이름 `Base`, 타입 `ns::Base`)을 기반 클래스 항목으로 판별합니다.
일반 멤버 이름까지 타입명과 동일한 특수한 경우는 구분에 한계가 있으므로 해당 객체를
루트로 직접 지정해 바인딩하세요. 단순히 `parent`라는 이름의 포인터를 모두 제외하는 규칙은 아닙니다.
...
같은 옵션은 **Debug dump: Search / Array tabs**로 표시되며 Array 추가/전환/닫기도
`array-tab-*.log`에 기록합니다. 문제가 난 순서대로 조작하면 이전/대상 탭 ID,
표현식, 저장된 타입과 크기, 설정 복원/줌/선택영역/통계 처리별 BEGIN/END,
...
**Save** 또는 Visual Studio 옵션 창의 **확인(OK)**으로 저장하면 같은 VS 인스턴스의
Viewer 템플릿 목록과 선택된 정의가 자동 갱신됩니다. **Reload templates**는 별도
수동 재읽기가 필요한 경우에만 사용합니다. 저장 자체는 검색이나 디버거 메모리 읽기를
시작하지 않으며 Search root, 현재 버퍼, 확대 배율을 바꾸지 않습니다.

Enter는 입력을 확정하고 옵션 페이지에 머뭅니다. **확인(OK)**은 저장 후 닫기,
**취소(Cancel)**는 마지막 저장 이후 편집을 버리기입니다. 필수 값 누락이나 중복된
타입명은 셀과 하단 상태줄에 표시하며 저장을 막습니다. Save 성공 팝업은 띄우지 않습니다.
설정 파일의 기존 형식/솔루션별 구분은 유지하며, 교체 저장 시 직전 파일은 `.bak`으로 남깁니다.
